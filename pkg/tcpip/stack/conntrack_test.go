...
func testWindowScaling(t *testing.T, windowSize uint16, synScale, synAckScale uint8, dataLen int, finalSeq uint32) {
	...
	synPkt.tuple = ct.getConnOrMaybeInsertNoop(synPkt)
	...
	synAckPkt.tuple = ct.getConnOrMaybeInsertNoop(synAckPkt)
	...
	ackPkt.tuple = ct.getConnOrMaybeInsertNoop(ackPkt)
...
func genTCPPacket(opts genTCPOpts) *PacketBuffer {
	...
	if opts.windowScale != 0 {
		scaleOpt := []byte{
			header.TCPOptionWS, // Kind.
			3,                  // Length.
			opts.windowScale,   // Shift count.
		}
		copy(tcpHdr[header.TCPMinimumSize:][:3], scaleOpt)
	}
